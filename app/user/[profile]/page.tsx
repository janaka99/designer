...
const page = (props: any) => {
  ...
  const getProjects = async () => {
    ...
    const res = await fetch(
      `/api/user/get/getId?email=${props.searchParams.email}`,
      {
        method: "GET",
      }
    );
  ...
  useEffect(() => {
    if (status === "authenticated") {
      router.push(`/in/${session?.user?.name}?user_id=${session?.user?.email}`);
    }
    ...
    <div className="py-10 max-w-[1580px] w-[95%] mx-auto flex flex-col justify-center items-center gap-10">
      <div className="flex flex-col justify-center items-center gap-5">
        <img
          ...
          alt=""
        />
        <div className="text-7xl text-center">{user?.username}</div>
        <a
          href={`/user/${user?.username}?email=${user?.email}`}
